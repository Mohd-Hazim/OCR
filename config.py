...
default_config = {
    ...
    # Manual override for tables (KEEP)
    "force_gemini_for_tables": False
}
...
# -------------------------------------------------------------
# === Gemini manual override ===
def get_force_gemini():
    """
    Return True if manual Gemini override is enabled.
    Safe fallback to False if missing.
    """
    return get_config_value("force_gemini_for_tables", False)


def set_force_gemini(value: bool):
    """
    Enable or disable manual Gemini override.
    Saves updated config to disk.
    """
    cfg = load_config()
    cfg["force_gemini_for_tables"] = bool(value)
    save_config(cfg)
    logger.info(f"Manual Gemini override set to: {value}")
